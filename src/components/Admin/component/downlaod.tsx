...
export const handleDownload = (order:any) => {
  ...
  doc.text(`PO Number: #${String(order.orderNumber || "NIL")}`, margin, currentY);
  currentY += 10;
  doc.text(`Order Date: ${String(order.createdAt || "NIL")}`, margin, currentY);
  currentY += 10;
  doc.text(
    `Estimated Dispatch Date: ${String(
      order.estimatedDispatchDate || "NIL"
    )}`,
    margin,
    currentY
  );
  ...
  doc.setFontSize(12);
  currentY += 10;
  doc.text(`Employee Name: ${String(order.generatedBy?.user?.username || "NIL")}`, margin, currentY);
  currentY += 10;
  doc.text(`Employee Id: ${String(order.generatedBy?.employeeId || "NIL")}`, margin, currentY);
  currentY += 10;
  doc.text(`Designation: ${String(order.generatedBy?.designation || "NIL")}`, margin, currentY);
  ...
  doc.setFontSize(12);
  orderThroughY += 10;
  doc.text(`Employee Name: ${String(order.orderThrough?.name || "NIL")}`, orderThroughX, orderThroughY);
  orderThroughY += 10;
  doc.text(`Employee Id: ${String(order.orderThrough?.id || "NIl")}`, orderThroughX, orderThroughY);
  orderThroughY += 10;
  doc.text(`Designation: ${String(order.orderThrough?.designation || "NIL")}`, orderThroughX, orderThroughY);
  ...
  doc.setFontSize(12);
  currentY += 10;
  doc.text(`Client Name: ${String(order.clientName || "NIL")}`, margin, currentY);
  currentY += 10;
  doc.text(`Company Name: ${String(order.companyName || "NIL")}`, margin, currentY);
  currentY += 10;

  // Split address into lines for wrapping
  const addressText = String(order.address || "NIL");
  ...
  doc.text(`Zipcode: ${String(order.zipCode|| "NIL")}`, margin, currentY);
  currentY += 10;
  doc.text(`Contact No.: ${String(order.contact || "NIL")}`, margin, currentY);
  currentY += 10;
  doc.text(`GST No.: ${String(order.gstNumber|| "NIL")}`, margin, currentY);
  ...
  products.forEach((product:any) => {
    ...
    doc.text(String(product.price || "NIL"), xPrice, currentY + verticalTextPadding);
