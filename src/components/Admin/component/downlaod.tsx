...
export const handleDownload = (order: any) => {
  ...
  // ----------------------------------------------------
  // --- UPDATED: Load logo and maintain aspect ratio ---
  // ----------------------------------------------------
  const logoUrl = "/images/logo.png"; // Assuming logo.png is in the public folder
  const logoDesiredHeight = 20; // Fixed height of the logo in mm
  ...
  img.onload = function () {
    ...
    const logoWidth = logoDesiredHeight * aspectRatio; // Calculate new width based on aspect ratio
    const logoHeight = logoDesiredHeight; // Use the desired height

    doc.addImage(img, "PNG", logoX, logoY, logoWidth, logoHeight);

    // Update starting Y position for company address to be below the logo
    const companyAddressCurrentY = logoY + logoHeight + 5; // Start address below the logo
    ...
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    let companyAddressTextY = companyAddressCurrentY + 5;
    doc.text("Naya Nagar, CDF Industrial Area, Anopshahar Road,", companyAddressStartX, companyAddressTextY);
    companyAddressTextY += 5;
    doc.text("Cherat, Aligarh, Uttar Pradesh 202002", companyAddressStartX, companyAddressTextY);
    companyAddressTextY += 5;
    doc.text("Email: qenggsolutions@gmail.com", companyAddressStartX, companyAddressTextY);
    companyAddressTextY += 5;
    doc.text("Phone: +91 8077774614", companyAddressStartX, companyAddressTextY);
    const companyAddressBottomY = companyAddressTextY; // Bottom edge of company address block
    ...
    currentY += 5;
    doc.text("2. If you want to delete this Purchase Order, You must ask permission from admin/sub-admin", sideMargin, currentY);
    // currentY += 5;
    // doc.text("1. If you want to update this Purchase Order, You must ask permission from admin/sub-admin", sideMargin, currentY);
  ...
  img.onerror = function () {
    ...
    alert("Error loading company logo. The PDF will be generated without it.");
  };
};