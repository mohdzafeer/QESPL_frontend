    ...
    <div className="lg:w-4xl xl:w-4xl w-sm lg:max-h-[600px] xl:max-h-[700px] max-h-[700px] overflow-y-auto overflow-x-hidden no-scrollbar rounded-xl bg-white dark:bg-zinc-800 dark:text-white py-5 px-10 flex flex-col">
        ...
        <div className="text-end flex flex-col w-full mt-5">
          ...
          <span className="text-xs font-semibold">
            ...
            <span className="font-bold">
              {formatDate(orderData.estimatedDispatchDate)}
            </span>{" "}
            {/* Display dynamic Estimated Dispatch Date */}
          </span>
          <span className="text-xs my-2">
            ...
            >
              ...
              {/* Display dynamic Status, uppercase */}
            </span>
          </span>
        </div>
      </div>
      ...
      <div className="flex justify-between mt-1 gap-4 flex-col lg:flex-row xl:flex-row ">
        ...
        <div className="flex flex-col text-sm text-start px-2 py-4 lg:pr-20 xl:pr-20 ">
          <span className="font-mono font-semibold w-full text-lg">
            Generated By
          </span>
          <img
          ...
          <span>
            ...
            <span className="font-semibold">
              {orderData.generatedBy?.username}
            </span>{" "}
          ...
          <span>
            ...
            <span className="font-semibold text-blue-500 underline">
              {orderData.generatedBy.employeeId
                ? orderData.generatedBy.employeeId
                : "_"}{" "}
              {/* Dynamic Employee ID */}
            </span>
      ...
      <div className="bg-gray-100 dark:bg-zinc-900 text-start px-2 py-4 text-sm mt-4 rounded-lg">
        ...
        <div className="flex justify-between flex-col lg:flex-row xl:flex-row overflow-x-auto">
          <div className="flex flex-col text-start">
            <span>
              Client Name :{" "}
              <span className="font-semibold">{orderData.clientName}</span>{" "}
              {/* Dynamic Client Name */}
            </span>
            <span>
              Company Name :{" "}
              <span className="font-semibold ">
                {orderData.companyName ? orderData.companyName : "_"}{" "}
            ...
            <span className="max-w-sm flex gap-2">
              ...
              <span className="font-semibold break-words">
                {orderData.address ? orderData.address : "_"}
              </span>{" "}
              {/* Dynamic Address */}
            </span>
          </div>
          ...
          <div className="flex flex-col text-start min-w-sm ">
            <span>
              Zipcode :{" "}
              <span className="font-semibold">
                {orderData.zipCode ? orderData.zipCode : "_"}
              </span>{" "}
              {/* Dynamic Zipcode */}
            </span>
            <span>
              Contact No. :{" "}
              <span className="font-semibold">
                {orderData.contact ? orderData.contact : "_"}
              </span>{" "}
              {/* Dynamic Contact No. */}
            </span>
            <span>
      ...
      <div className="flex justify-between items-center mt-4">
        <button
          onClick={onClose}
          className="px-3 py-2 bg-gray-300 rounded-lg text-black text-sm max-w-fit flex items-center gap-4 font-semibold cursor-pointer hover:bg-gray-200 duration-300 active:bg-gray-600"
        >
          Close
        </button>
        {loading === false ? (
          <button
            onClick={() => {
              loadingFun(), handleDownload(orderData);
            }}
